...
model1e <- readRDS("Fight Tables/coxme_calendar_year.RDS")

model1f <- readRDS("Fight Tables/coxme_calendar_year_tercile.RDS")
...
addtorow$command <- c('\\hline',
                      ...
                      'strata(region) FE & No & No & No & Yes & Yes & Yes \\\\\n',
...
# Sex + strata(region) + Calendar Year Tercile ------------------------------------
# PH violated for tercile
model4 <- coxme(Surv(exit, tree.fall.during.interval) ~ strata(male) + strata(region) +
