...
stargazer(model5a, type = "latex", report = "vcsp", single.row = T, title = "Animal Attack (c) \\vspace{-1.4em}",
          ...
          dep.var.labels = "Hazard Rate",
          covariate.labels = c("Length of Prior Animal Attack (c) Interval"),
...
stargazer(model5b, type = "latex", report = "vcsp", single.row = T, title = "Animal Attack (c) \\vspace{-1.4em}",
          ...
          dep.var.labels = "Hazard Rate",
          covariate.labels = c("Length of Prior Animal Attack (c) Interval", "Male"),
          add.lines = list(c("No. of Individuals", "182"),
                           c("No. of Intervals", "280"),
                           c("No. of Risk Years", "6,753.24")),
          omit.stat = c("ll", "n", "rsq", "max.rsq", "wald", "lr", "logrank"))
...
# summary(model6d)
# Note that Animal Attack Combined (c) does not violate proportional hazards for region, test is
# significant
...
addtorow$command <- c('\\hline',
                      ...
                      '\\hline')
print(xtable(df_coxme, caption = "Animal Attack (c) Mixed Effects Specifications"), add.to.row = addtorow,
      caption.placement = "top", file = "Animal Attack Combined Tables/model6.tex")
...
# Survival risk table
fit <- survfit(Surv(enter, exit, Animal_Attack.during.interval) ~ 1, df_first)
